...
//------------------------------------------------------------------------------------------------------------------------------//
$query = mysqli_query($conn, "SELECT mnum, email FROM `user_table` WHERE mnum LIKE '%{$phone}%' OR email LIKE '%{$email}%'");
...
if($rows)
{
  ...
  Swal.fire({
    icon: 'error',
    text: 'This info already exist on  system, Please check your profile or update your details ',
    confirmButtonText: 'OK',
    confirmButtonColor: '#3085d6',

  }).then(function(){
    window.location.href='http://localhost/DataMaster/Admin_Dash-board/pages/index.php';
  });
  </script>");
}
else{
//------------------------------------------------------------------------------------------------------------------------------//
  ...
  if($result){
    ...
    Swal.fire({
      ...
      window.location.href='http://localhost/DataMaster/Datamaster-User-Side/Record.html';
    });
    </script>");
  }
}
